...
function getStoryClient(): StoryClient {
  ...
  const account = privateKeyToAccount(
    `0x${config.ethereum.privateKey.replace("0x", "")}` as `0x${string}`
  );
  ...
  if (!chainId) {
    throw new Error(
      'STORY_PROTOCOL_CHAIN_ID is required. Use "aeneid" for testnet or "sonic" for mainnet'
    );
  }
...
export class StoryProtocolService {
  ...
  async checkBalance(): Promise<{
    ...
    try {
      ...
      throw new Error(
        `Failed to check wallet balance: ${error.message || error}`
      );
    }
  }

  async registerIP(ipMetadata: {
    name: string;
    description?: string;
    mediaUrl: string;
    thumbnailUrl?: string;
    ...
    try {
      ...
      logger.info("Checking wallet balance before registration", {
        address: account.address,
      });
      ...
      try {
        ...
        console.log(
          `💰 Wallet balance: ${balanceCheck.balanceFormatted} tokens`
        );
        ...
        logger.info(
          `Wallet has sufficient balance: ${balanceCheck.balanceFormatted} tokens`
        );
      } catch (balanceError: any) {
        // If balance check fails, log warning but continue (might be RPC issue)
        logger.warn(
          "Balance check failed, proceeding anyway:",
          balanceError.message
        );
        console.warn(
          "⚠️  Could not check balance, proceeding with registration attempt..."
        );
      }
      ...
      const ipIpfsResult = await uploadJSONToIPFS(
        ipMetadataObj,
        `ip-metadata-${Date.now()}.json`
      );
      const nftIpfsResult = await uploadJSONToIPFS(
        nftMetadata,
      ...
      const spgNftContract =
        process.env.STORY_PROTOCOL_SPG_NFT_CONTRACT ||
      ...
      if (
        ...
        errorMessage =
          `Insufficient funds on Aeneid testnet. Wallet ${account.address} needs testnet tokens to pay for gas. ` +
  ...
  async createRoyaltyToken(params: {
    ...
    try {
      logger.info("Creating royalty token tracking for IP", {
        ipId: params.ipId,
      });
      ...
      const tokenAddress = `0x${Buffer.from(
        `${params.ipId}-royalty-${Date.now()}`
      )
        .toString("hex")
        .slice(0, 40)
        .padEnd(40, "0")}`;
      ...
      logger.info(
        "Royalty token tracking created. Story Protocol handles actual royalty distribution via PIL terms."
      );
      ...
      throw new Error(
        `Failed to create royalty token: ${error.message || error}`
      );
    }
  }
  ...
  async getRoyaltyTokenBalance(params: {
    ...
    try {
      ...
      throw new Error(
        `Failed to get royalty token balance: ${error.message || error}`
      );
    }
  }
  ...
  async transferRoyaltyTokens(params: {
    ...
    try {
      ...
      throw new Error(
        `Failed to transfer royalty tokens: ${error.message || error}`
      );
    }
  }
   ...
   */
  async getRoyaltyRevenue(params: { ipId: string }): Promise<{
    totalRevenue: bigint;
    currency: string;
    ...
    try {
      ...
      logger.info(
        "Royalty revenue retrieved. Story Protocol handles distribution automatically via PIL terms."
      );
      ...
      throw new Error(
        `Failed to get royalty revenue: ${error.message || error}`
      );
    }
  }
  ...
  async claimAllRevenue(params: {
    ...
    try {
      ...
      const response = await client.royalty.claimAllRevenue({
        ...
        royaltyPolicies: (params.royaltyPolicies || []).map(
  ...
  async registerDerivative(params: {
    ...
    try {
      ...
      if (params.derivedMetadata) {
        ...
        const response = await client.ipAsset.registerIpAsset({
          ...
          ipMetadata: {
            ...
            ipMetadataHash: ("0x" +
              createHash("sha256")
                .update(params.derivedMetadata.mediaUrl)
                .digest("hex")) as `0x${string}`,
            nftMetadataURI:
              params.derivedMetadata.thumbnailUrl ||
              params.derivedMetadata.mediaUrl,
            nftMetadataHash: ("0x" +
              createHash("sha256")
                .update(
                  params.derivedMetadata.thumbnailUrl ||
        ...
        return {
          ...
          licenseId: `0x${Buffer.from(
            `${params.parentIPId}-${
              response.ipId || params.derivedIPId
            }-${Date.now()}`
          )
            .toString("hex")
            .slice(0, 40)}`,
      ...
      logger.warn(
        "Derivative registration: Using placeholder (metadata not provided)"
      );
      return {
