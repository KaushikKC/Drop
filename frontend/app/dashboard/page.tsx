...
import {
  getCreatorDashboard,
  /* getUserLicenses, */ getCreatorTransactions,
  getRoyaltyToken,
  createRoyaltyToken,
  getRoyaltyRevenue,
  type RoyaltyToken,
...
const Dashboard = () => {
  ...
  const [royaltyTokens, setRoyaltyTokens] = useState<Map<string, RoyaltyToken>>(
    new Map()
  );
  const [royaltyLoading, setRoyaltyLoading] = useState<Map<string, boolean>>(
    new Map()
  );
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      try {
        ...
        try {
          const transactionsData = await getCreatorTransactions(
            wallets[0].address
          );
          ...
          console.log("📊 Transactions fetched:", {
            ...
            received: allTransactions.filter(
              (t: Transaction) => t.type === "received"
            ).length,
            sent: allTransactions.filter((t: Transaction) => t.type === "sent")
              .length,
          ...
          const receivedTxs = allTransactions.filter(
            (tx: Transaction) => tx.type === "received"
          );
          const revenue =
            receivedTxs.reduce((sum: number, tx: Transaction) => {
              const amount = parseFloat(tx.amount || "0");
              console.log(
                `💰 Adding revenue from transaction ${tx.id}: ${amount} USDC (type: ${tx.type})`
              );
              return sum + amount;
            }, 0) || 0;

          console.log(
            "💰 Total License Revenue calculated:",
            revenue,
            "USDC",
            `(from ${receivedTxs.length} received transactions)`
          );
          ...
          if (asset.story_ip_id) {
            try {
              ...
              if (royaltyToken) {
                ...
                totalRoyalty +=
                  parseFloat(royaltyToken.totalRevenue || "0") / 1e6; // Convert from wei to USDC
              }
            } catch (error) {
              console.error(
                `Failed to fetch royalty token for asset ${asset.id}:`,
                error
              );
            }
          }
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <div>
            ...
            <p className="text-gray-500 font-medium text-sm">
              Overview of your IP assets and transactions.
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          <div className="bg-white border border-gray-100 p-8 rounded-2xl shadow-sm hover:shadow-lg transition-all">
            ...
            <p className="text-xs text-gray-500">
              Assets you've created and uploaded
            </p>
          </div>
          <div className="bg-white border border-gray-100 p-8 rounded-2xl shadow-sm hover:shadow-lg transition-all">
            <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-2">
            ...
            <p className="text-xs text-gray-500">
              Total earnings from license sales
            </p>
          </div>
          <div className="bg-white border border-gray-100 p-8 rounded-2xl shadow-sm hover:shadow-lg transition-all">
            ...
            <h3 className="text-3xl font-black text-[#0F172A] mb-2">
              {loading ? (
                ...
                `${totalRoyaltyRevenue.toFixed(2)} USDC`
              )}
            </h3>
            <p className="text-xs text-gray-500">
              Earnings from derivative works
            </p>
          </div>
        </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-white border border-gray-100 rounded-2xl">
              ...
              <p className="text-sm text-gray-400">
                Transactions will appear here when someone purchases your assets
                or when you purchase licenses
              </p>
            </div>
          ) : (
                          ...
                          {tx.type === "received" ? (
                            <div>
                              ...
                              <p
                                ...
                                title={tx.buyer.address}
                              >
                                {formatAddress(tx.buyer.address)}
                              </p>
                            </div>
                          ) : (
                            <div>
                              ...
                              <p
                                className="text-sm font-mono text-gray-700"
                                title={
                                  tx.asset.creatorAddress || tx.seller.address
                                }
                              >
                                {formatAddress(
                                  tx.asset.creatorAddress || tx.seller.address
        ...
        <div className="mb-12">
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-white border border-gray-100 rounded-2xl">
              ...
              <p className="text-sm text-gray-400">
                Upload your first asset to start monetizing
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {createdAssets.map((asset) => (
                <div
                  key={asset.id}
                  className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all group cursor-pointer"
                >
                  ...
                  <div className="p-4">
                    <h4 className="text-sm font-bold text-[#0F172A] mb-1 truncate">
                      {asset.title}
                    </h4>
                    <p className="text-xs text-gray-500 font-medium mb-2">
                      {asset.price
                        ? `${asset.price} ${asset.currency}`
                        : `${Number(asset.price_wei) / 1e6} ${asset.currency}`}
                    </p>
                    {asset.story_ip_id && (
                      <>
                        <a
                            ...
                            <div className="space-y-1">
                              <div className="flex items-center justify-between">
                                ...
                                <span className="text-xs font-bold text-[#0033FF]">
                                  {royaltyTokens.get(asset.id)?.tokenSymbol ||
                                    "RRT"}
                                </span>
                              </div>
                              <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <span className="text-xs font-medium text-gray-700">
                                  {royaltyTokens.get(asset.id)?.holdersCount ||
                                    0}
                                </span>
                              </div>
                            </div>
                            ...
                            <button
                              onClick={async (e) => {
                                ...
                                setRoyaltyLoading((prev) =>
                                  new Map(prev).set(asset.id, true)
                                );
                                try {
                                  const token = await createRoyaltyToken(
                                    asset.id
                                  );
                                  setRoyaltyTokens((prev) =>
                                    new Map(prev).set(asset.id, token)
                                  );
                                  setTotalRoyaltyRevenue(
                                    (prev) =>
                                      prev +
                                      parseFloat(token.totalRevenue || "0") /
                                        1e6
                                  );
                                } catch (error: any) {
                                  console.error(
                                    "Failed to create royalty token:",
                                    error
                                  );
                                  alert(
                                    error.message ||
                    ...
                    {asset.ipfs_cid && (
                      <p
                        className="text-xs text-gray-400 font-mono mt-1 truncate"
                        title={asset.ipfs_cid}
                      >
